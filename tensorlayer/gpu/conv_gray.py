# -*- coding: utf-8 -*-
"""
Created on Thu Mar 23 11:37:19 2017
...
#folder="G:\CLS\dissertation\dataset"
extension=".jpg"
inputSize=50

def getData(folder,extension,size):
################ read file
    print("getData, folder:",folder)
    file_list=[]
    ...
    data=np.zeros([num,size,size],np.float32)
    for i in range(num):
        im=io.imread(file_list[i])
        im=transform.resize(rgb2gray(im), (size, size))
        im = np.copy(im).astype('float32')
#       io.imshow(im)
#    tempData.resize(1,500*500)
        data[i,:,:]=im[:,:]
        if i%20 == 0 :
            print("getData, file index:",i,",total:",num)
    return data


# Data loading and preprocessing

Xdental = getData("N:/trainImg/gray/dentalCT",extension,inputSize)
Xskull = getData("N:/trainImg/gray/skullCT",extension,inputSize)
Xtooth = getData("N:/trainImg/gray/toothCT",extension,inputSize)
Ydental = np.zeros([len(Xdental),1],np.int64)
Ydental[:,0]=0
Yskull = np.zeros([len(Xskull),1],np.int64)
Yskull[:,0]=1
Ytooth = np.zeros([len(Xtooth),1],np.int64)
Ytooth[:,0]=2

X_train = np.row_stack([Xdental[0:1000,:,:], Xskull[0:90,:,:], Xtooth[0:2000,:,:]]) 
y_train = np.row_stack([Ydental[0:1000,:], Yskull[0:90,:], Ytooth[0:2000,:]]) 
X_test = np.row_stack([Xdental[len(Xdental)-30:len(Xdental),:,:], Xskull[91:len(Xskull),:,:], Xtooth[len(Xtooth)-30:len(Xtooth),:,:]]) 
y_test = np.row_stack([Ydental[len(Xdental)-30:len(Xdental),:], Yskull[91:len(Xskull),:], Ytooth[len(Xtooth)-30:len(Xtooth),:]]) 
...
def read_and_decode(filename, is_train=None):
    ...
    img = tf.reshape(img, [inputSize, inputSize, 1])
    # img = tf.cast(img, tf.float32) #* (1. / 255) - 0.5
    if is_train == True:
        # 1. Randomly crop a [height, width] section of the image.
        img = tf.random_crop(img, [inputSize-5, inputSize-5, 1])
        ...
        img = tf.image.resize_image_with_crop_or_pad(img, inputSize-5, inputSize-5)
...
data_to_tfrecord(images=X_train, labels=y_train, filename="train.gray")
data_to_tfrecord(images=X_test, labels=y_test, filename="test.gray")
...
batch_size = 16
model_file_name = "model_gray_advanced.ckpt"
resume = False # load model, resume from previous checkpoint?

with tf.device('/cpu:0'):
    sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) #
    # prepare data in cpu
    x_train_, y_train_ = read_and_decode("train.gray", True)
    x_test_, y_test_   = read_and_decode("test.gray", False)
    ...
    def inference(x_crop, y_, reuse):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            network = tl.layers.Conv2dLayer(network, act=tf.nn.relu,
                        shape=[5, 5, 1, 64], strides=[1, 1, 1, 1], padding='SAME', # 64 features for each 5x5x3 patch
    ...
    with tf.device('/cpu:1'): # <-- remove it if you don't have GPU
        # network in gpu
